...
namespace FhirStarter.STU3.Detonator.DotNetCore.SparkEngine.Formatters
{
    public class HtmlFhirFormatter : FhirMediaTypeFormatter
    {
        ...
        private void WriteHtmlOutput(Type type, object value, Stream writeStream)
        {
            ...
            else if (type == typeof(Resource))
            {
                ...
                else
                {
                    ...
                    var xslt = GetRenderXMLAsHTMLXslt();
                    //var stylesheetReader = System.Xml.XmlReader.Create(new StringReader(Resources.RenderXMLasHTML));
                    if (string.IsNullOrEmpty(xslt))
                    {
                        throw new ArgumentException("The embedded resource renderxmlashtml.xslt could not be found");
                    }
                    var stylesheetReader = System.Xml.XmlReader.Create(new StringReader(xslt));
        ...
        private string GetRenderXMLAsHTMLXslt()
        {
            var assembly = typeof(HtmlFhirFormatter).GetTypeInfo().Assembly;
            // assuming there is only one xslt
            var manifest = assembly.GetManifestResourceNames().FirstOrDefault(p => p.Contains(".xslt"));
